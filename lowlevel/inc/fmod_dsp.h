...
[
    [DESCRIPTION]
    Default behaviour helper, for a FMOD_DSP_SHOULDIPROCESS_CALLBACK callback if no tail is required for a DSP effect when the signal goes idle.

    [REMARKS]
    The FMOD_DSP_SHOULDIPROCESS_CALLBACK callback is specified to tell FMOD's mixer whether the FMOD_DSP_READ_CALLBACK callback should be called or not.  This can be used as an optimization to reduce CPU overhead.
    The helper provided here gives a simple default to disable an effect if the incoming signal goes idle, due to a sound stopping for example.
    If the effect does not need to continue processing silence, then FMOD_ERR_DSP_DONTPROCESS can be returned in the FMOD_DSP_SHOULDIPROCESS_CALLBACK callback to skip the FMOD_DSP_READ_CALLBACK callback.
    
    An example of an effect that would continue processing silence would be an echo or reverb effect that needs to play a tail sound until it fades out to silence.  At that point it could return 
    FMOD_ERR_DSP_DONTPROCESS as well.
    This is a default that can be used to immediately detect no signal coming into the effect and return that it does not want to process the effect.

    NOTE: Effects that no not stop processing may keep the signal chain alive when it is not desirable to do so.  FMOD Studio events may return that they are still playing when they should be stopped.
...
*/
static FMOD_RESULT F_CALLBACK FMOD_ShouldIProcess_NoTail(FMOD_DSP_STATE *dsp_state, bool inputsidle, unsigned int length, FMOD_CHANNELMASK inmask, int inchannels, FMOD_SPEAKERMODE speakermode)
{
    if (inputsidle)
    {
